...
public class SprdNatTimeUtils {
    ...
    SprdNatTimeUtils(Context context) {
        ...
        mccMnc = mTelephonyManager.getNetworkOperatorForSubscription(dataSubId);
        mNetworkClass = TelephonyManager.getNetworkClass(mTelephonyManager
                .getNetworkType(dataSubId));
        mConfigManager = (CarrierConfigManager) context
    ...
    public int getNatOvertimeByNetClass(int networkClass, int subId) {
        ...
        switch (networkClass) {
            case TelephonyManager.NETWORK_CLASS_2_G:
                return persistableBundle.getInt(CarrierConfigManager.KEY_NETWORK_NAT_OVERTIME_2G);
            case TelephonyManager.NETWORK_CLASS_3_G:
                return persistableBundle.getInt(CarrierConfigManager.KEY_NETWORK_NAT_OVERTIME_3G);
            case TelephonyManager.NETWORK_CLASS_4_G:
                return persistableBundle.getInt(CarrierConfigManager.KEY_NETWORK_NAT_OVERTIME_4G);
            default:
    ...
    BroadcastReceiver mTelephonyReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (subId == SubscriptionManager.getDefaultDataSubId()) {
                if (action == TelephonyIntents.ACTION_SERVICE_STATE_CHANGED) {
                ...
                } else if (action == CarrierConfigManager.ACTION_CARRIER_CONFIG_CHANGED) {
                    newMccMnc = mTelephonyManager.getNetworkOperatorForSubscription(subId);
