...
final class AlarmAlignHelper{
    ...
    private PowerGuru mPowerGuruService;
    private List<PowerGuruAlarmInfo> mBeatlist;

    public AlarmAlignHelper(Context context) {
        mPowerGuruService = (PowerGuru)context.getSystemService(Context.POWERGURU_SERVICE);
        updateBeatlist();
    ...
    protected PowerGuruAlarmInfo matchBeatListPackage(final PendingIntent pi){
        if(mBeatlist != null && mBeatlist.size() > 0){
